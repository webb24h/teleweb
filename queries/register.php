 ...
 */
if (!empty($_POST)) {
    if (isset($_POST['first_name']) && $_POST['last_name'] && $_POST['mobilePhone'] && $_POST['email'] && $_POST['username'] && $_POST['password'] && $_POST['countryCode'] && $_POST['company'] && $_POST['website'] && $_POST['industry']
    ) {
        ...
        if ($user_result->num_rows > 0) {//there is a record
            ...
            if ($checkuser['isActive'] == 1) {//account is active
                ...
                header("Location: ../index.php?login&errorRegistrationActive");
            } else {//redirect them to validation page
                //get user id
                $user_id = $checkuser['uid'];


                //redirect to validate identity to activate account
                header("Location: ../index.php?validate=$user_id&pleaseValidate");
            }
        } else {
            ...
            $insertuserquery = $conn->prepare("INSERT INTO user (user_level,industry,website,company,username,first_name,last_name,email,mobile,password,date_created) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
            $insertuserquery->bind_param("sssssssssss", $user_level,$industry, $website, $company, $username, $first_name, $last_name, $email, $phoneNumber, $passwordHash, $current_date);
        ...
        if ($uservalida_stmt_finishs == 1) {
            ...
            $body = "
                ...
                Welcome to Teleweb!
                <br><br>
                No more pricey landline. No more hassle.
                <br><br>
                Teleweb is the last call center software you will ever need. 
                It is the only solution out there that is completely wireless and hosted online. 
                No need for landline or Voip. All you need is your computer and a headset. 
                <br><br>
                Perfect for in-house and remote teams, no matter how big or small.
                <br><br>
                Teleweb is your go-to call center software.
                <br><br>
                Login to your account now and enjoy all the benefits.
                <br><br>
                Password : $password   
                <br><br>
...
// Use the client to do fun stuff like send text messages!
        ...
        if ($ok == 3) {
            //redirect to validate identity to activate account
            header("Location: ../index.php?validate=$last_id&validateIdentity");
        }
    }
}